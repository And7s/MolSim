...
BoundaryCondition::BoundaryCondition(LCDomain* linkedCell_,
		std::vector<int> domainSize_, int dimension_, auto_ptr<input_t>& inp,
		int* length_) {
...
void BoundaryCondition::applyReflecting(ParticleContainer* pc, int axis,
		bool zero) {
	Particle* p;
	int j = 0;
	while ((p = pc->nextParticle(&j)) != NULL) {
		if (p->getType() != -1) {
			...
			if (zero) {
				...
				if (oldX[axis]
						>= domainSize[axis] * linkedCell->getCutOffRadius()
								+ distance) {
					std::cout << "Distance "
							<< domainSize[axis] * linkedCell->getCutOffRadius()
									+ distance << " " << *p << "\n";
					...
					oldX[axis] = domainSize[axis]
							* linkedCell->getCutOffRadius() + distance;
				}
...
void BoundaryCondition::applyPeriodic(ParticleContainer* pc,
		ParticleContainer* pc2, int axis, bool zero,
		std::vector<int>& posCell) {
...
void BoundaryCondition::applySwitch(int type, std::vector<int>& pos, int axis,
		bool zero) {
	...
	if (type == 0) {
		...
		LOG4CXX_FATAL(loggerBoundaryCondition,
				"undefined BoundaryCondition called");
	}
...
void BoundaryCondition::apply() {
	...
	for (int i = 0; i < domainSize[0] + 2; i++) {
		...
		for (pos[1] = 0; pos[1] < domainSize[1] + 2; pos[1]++) {
			for (pos[2] = 0;
					pos[2] < domainSize[2] + 2 && (pos[2] == 0 || dimension > 2);
					pos[2]++) {
				if (pos[0] == 0) {
					//check left
