                                        ...
                                        while((curP = neighboursOfPc[j]->nextParticle(&interactingParticlesIt))!=NULL){
                                                ...
                                                if(p->approxDist(curP,cutHalf)){ //improves speed by about 4% + inline 1%
                                                        ...
                                                        if((distSq<=cutoffSq)&&(distSq>0)){
                                                                ...
                                                                /*if(isnan(forceIJ[0]) || isnan(forceIJ[1]) || isnan(forceIJ[2])) {
                                                                    std::cout << "NANA\n";
                                                                    std::cerr << "factor1"<<factor1<<" distSq "<<distSq<<" factor2 "<<factor2<<"\n";
                                                                    std::cerr << *p << "\n"<<*curP;
                                                                    exit(0);
                                                                }*/
                                                                if(p->getUid() == 0) {
                                                                    //  std::cerr << " 0 mit "<<curP->getUid()<< " typ "<<curP->getType()<<" \n";
                                                                }
                                                               /* if(forceIJ.L2Norm() > 1000000) {
                                                                    std::cerr << "====High force: \nfactor1"<<factor1<<" distSq "<<distSq<<" factor2 "<<factor2<<"\n";
                                                                    
                                                                    std::cerr << "distsq "<<distSq<<" kleiner als "<<cutoffSq<<"\n";
                                                                    std::cerr<< "Engery "<< forceIJ.L2Norm()<<"\n"<<*p<<" \n"<<*curP<<"\n";
                                                                }*/
